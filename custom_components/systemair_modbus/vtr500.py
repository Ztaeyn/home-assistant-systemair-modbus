"""Systemair SAVE VTR 500 model (register map + derived values).

Generated from the last known 100% working "gammel" version.
Internal keys are taken from old unique_id/name (without vtr500_ prefix).
"""
...
class VTR500Model:
    model_id = "vtr500"
    model_name = "Systemair SAVE VTR 500"
    ...
    def compute_derived(data: dict[str, Any]) -> dict[str, Any]:
        ...
        out["active_season"] = "Sommer" if season == 0 else "Vinter" if season == 1 else "Ukjent"
        ...
        try:
            ...
            v = None
        if v is None:
            out["iaq_level_text"] = "Ukjent"
        else:
            out["iaq_level_text"] = {0: "Økonomi", 1: "God", 2: "Forbedres"}.get(v, f"Ukjent ({v})")

        reg_mode = int(data.get("supply_air_room_exhaust_reg") or 0)
        if reg_mode == 0:
            out["regulation_mode_text"] = "Tilluftsregulering"
        elif reg_mode == 1:
            out["regulation_mode_text"] = "Romregulering"
        elif reg_mode == 2:
            out["regulation_mode_text"] = "Avtrekksregulering"
        else:
            out["regulation_mode_text"] = "Ukjent"

        mode = int(data.get("mode_status_register") or 0)
        man = int(data.get("manual_mode_command_register") or 0)

        if mode == 0:
            out["mode_status_text"] = {2: "Auto - Lav", 3: "Auto - Normal", 4: "Auto - Høy"}.get(man, "Auto - Normal")
        elif mode == 1:
            out["mode_status_text"] = {
                ...
                3: "Manuell - Normal",
                4: "Manuell - Høy",
            }.get(man, "Manuell - Ukjent")
        else:
            out["mode_status_text"] = VTR500Model.STATUS_MODE_TO_LABEL.get(mode, "Ukjent")
